...
書籍の章や解説文を想定して書かれた規範。Zenn 記事に適用するにあたっての調整は次のとおり。
SKILL.md はこの規範を Zenn 向けに要約したものなので、まずそちらを読む。ここは網羅的に点検したいときに開く。

## Zenn 向けの調整

- **一文ごとの改行は適用しない**。Zenn は単一改行を `<br>` として描画するため、規範どおりに改行すると本文が一文ごとに割れて見える。段落の分け方（1段落1トピック）だけを守る
- **脚注の代わりに Zenn 独自記法を使ってよい**。`:::message` `:::message alert` `:::details` のほうが読者に届く
- **敬体（です・ます）で書く**。ただし箇条書きは常体にする。textlint の `preset-ja-technical-writing` がそう要求する
- **中黒の禁止はコードブロック内に及ばない**。コードコメントは対象外とする
- **見出しのセリフ化**は、症状や対象を指すものなら許容する。結論や手順を宣言する見出しは避ける
- **口語の混入**は、筆者の感情や作業の手触りを書く箇所に限って許容する。事実と論証にかかる箇所では規範を優先する
- **textlint が機械的に検出する項目**（一文の長さ、読点の数、ですます統一、弱い表現、絶対表現、二重助詞）は、この規範で二度見しない
...
- 一文ごとに改行する。段落の区切りは空行で示す。（**Zenn では適用しない**。単一改行が `<br>` になる）
- コード、差分、ログ、設定ファイルの断片はコードブロックで示す。
- 用語の由来や定式化の名称など、本筋から一段外れる補足は、本文に並べず脚注（`[^ラベル]`）に降ろす。（**Zenn では `:::message` `:::details` を使ってよい**）
